...
// Win64:
	...
	// ldc nonsense
	// TODO: figure out why it's importing this from object.d even though -betterC?
	version (LDC) {} else {
	...
	void va_arg(T)(va_list apx, ref T parmn) {
		...
		static if (is(T U == __argTypes)) {
			static if (U.length == 0 || T.sizeof > 16 || (U[0].sizeof > 8 && !isVectorType!(U[0]))) {
				...
				static if ((is(T1 == double) || is(T1 == float)) && (is(T2 == double) || is(T2 == float))) {
					...
					void* a = void;
					if (ap.offset_fpregs < (6 * 8 + 16 * 8) && ap.offset_regs < 6 * 8 && T2.sizeof <= 8) {
						*cast(T1*)&parmn = *cast(T1*)(ap.reg_args + ap.offset_fpregs);
						ap.offset_fpregs += 16;
					...
					if (ap.offset_regs < 6 * 8 && T1.sizeof <= 8 && ap.offset_fpregs < (6 * 8 + 16 * 8)) {
